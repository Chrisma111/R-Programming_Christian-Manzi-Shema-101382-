---
title: "MANZI SHEMA CHRISTIAN/Data Science"
author: "ID101382"
...
world_population[] <- lapply(world_population, function(x) {
  if (is.character(x)) iconv(x, from = "Windows-1252", 
                             to = "UTF-8") else x})
...
```
```{r}
time_var<-select(world_population,Country.Territory,X2022.Population,Growth.Rate)
head(time_var)
```
...
```
```{r}
top10_CO2_emission <- CO2_emission[CO2_emission$`Country Name` %in% c("Mexic
o", "Bangladesh","Brazil","Nigeria","Pakistan","China","India","United States","Indonesia","Russian Federation" ), ]
...
#Change the duplicate column name
names(top10_CO2_emission)[names(top10_CO2_emission)== "2019...34"] <- "2019_A"
names(top10_CO2_emission)[names(top10_CO2_emission)== "2019...35"] <- "2019_B" 
data <- top10_CO2_emission

# Reshape data from wide to long
data_long <- data %>%
  pivot_longer(
    cols = matches("^[0-9]{4}"),# selects columns like 1990, 1991, ...
    names_to = "Year",
...
ggplot(data_long, aes(x = Year, y = CO2_Emissions, color = `Country Name`)) +
  geom_line(size = 1) +
  labs(title = "CO??? Emission Trends (1990-2019)",
    x = "Year",
    y = "CO??? Emissions"
  ) + theme_minimal()
...
cor_matrix <- cor(world_population[, c("Area..kmÂ..", "Density..per.kmÂ..","Growth.Rate", "World.Population.Percentage")])
print(cor_matrix)
...
```{r}
#Rename the column of both dataset
names(CO2_emission)[names(CO2_emission) == "Country Name"] <- "Country"
names(world_population)[names(world_population) == "Country.Territory"] <- "Country"
names(CO2_emission)[names(CO2_emission)== "2019...34"] <- "2019_A"
names(CO2_emission)[names(CO2_emission)== "2019...35"] <- "2019_B" 

# Merge datasets on country name
merged_data <- world_population %>%
inner_join(CO2_emission, by = c("Country" = "Country"))
...
extract_data1 <- merged_data %>%
  select(`Country`,`X2022.Population`,`2019_B`)

names(extract_data1)
```
```{r}
colSums(is.na(extract_data1))
```
```{r}
extract_clean <- na.omit(extract_data1)
head(extract_clean)
```
```{r}
# Statistical correlation
correlation_value <- cor( extract_clean[["X2022.Population"]], extract_clean[["2019_B"]], method ="pearson")
...
model<-lm(`2019_B` ~ X2022.Population, data = extract_clean)
summary(model)
```
...
ggplot(extract_clean, aes(x = X2022.Population, y = `2019_B`)) +
...
```{r}
ggplot(extract_clean, aes(x = log10(X2022.Population), y = log10(`2019_B`))) +
...
rename(Country ="Country", Continent = "Continent", Population_2022 ="X2022.Population", CO2_2019 = "2019_A"
)
...
```
1. What is the first continent with high 2019 CO2 emission = Europe 2. What is the third continent
